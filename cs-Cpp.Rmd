...
knitr::knit_engines$set(Cpp = function(options) {
    ...
    system2('echo', c(shQuote(code), ' > ', filename), stdout = TRUE)
    
    exec <- system2('/usr/bin/c++', c(filename, ' -o ', execname, ' -std=c++17'), stdout = TRUE, stderr = TRUE)

    if(is.null(attr(exec, "status"))) {
        exec <- system2(execname, stdout = TRUE, stderr = TRUE)
